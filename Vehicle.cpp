...
{
    std::list<std::pair<std::string, long>>::const_iterator cursor = route->begin();
    std::list<std::pair<std::string, long>>::const_iterator end = route->end();
...
void Vehicle::setRoute(std::list<std::pair<std::string, long>> &newRoute)
{
    if(!newRoute.empty())
    {
        route = new std::list<std::pair<std::string, long>>(newRoute);
    }
}